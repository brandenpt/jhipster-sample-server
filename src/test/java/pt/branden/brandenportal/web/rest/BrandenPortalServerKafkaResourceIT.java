...
import pt.branden.brandenportal.config.KafkaProperties;
...
class BrandenPortalServerKafkaResourceIT {
    ...
    void setup() {
        ...
        BrandenPortalServerKafkaResource kafkaResource = new BrandenPortalServerKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/branden-portal-server-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/branden-portal-server-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
